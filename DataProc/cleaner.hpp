...
class cleaner{
  ...
  public:
    ...
    cleaner(string _oldir, string _newdir){
      ...
      ofstream asamples{"apple_samples.csv"};
      appledkl.domainsamps.save(asamples,arma::csv_ascii);
      arma::mat temp = appledkl.ps_i.t();
      temp.save(asamples,arma::csv_ascii);
      asamples.close();
      ofstream bsamples;
      bsamples.open("banana_samples.csv");
      bananadkl.domainsamps.save(bsamples,arma::csv_ascii);
      temp = bananadkl.ps_i.t();
      temp.save(bsamples,arma::csv_ascii);
      bsamples.close();
      ofstream hsamples;
      hsamples.open("house_samples.csv");
      housedkl.domainsamps.save(hsamples,arma::csv_ascii);
      temp = housedkl.ps_i.t();
      temp.save(hsamples,arma::csv_ascii);
      hsamples.close();
      ofstream usamples;
      usamples.open("umbrella_samples.csv");
      umbrelladkl.domainsamps.save(usamples,arma::csv_ascii);
      temp = umbrelladkl.ps_i.t();
      temp.save(usamples,arma::csv_ascii);
      usamples.close();
    };
