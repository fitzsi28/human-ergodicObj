...
{   double xbound = 0.5,ybound = 0.5;
    ...
    SingleInt syst1 (1./60.);
    syst1.Ucurr = unom(0); 
    random_device rd; mt19937 eng(rd());
    uniform_real_distribution<> distr(-0.4,0.4);
    //syst1.Xcurr = {-0.2,0.0,0.1,0.0};
    syst1.Xcurr = {distr(eng),distr(eng)};//must be initialized before instantiating cost
    arma::mat R = 0.0001*arma::eye(2,2); double q=1.;
    arma::vec umax = {40.0,40.0};
    double T = 0.6;
    arma::mat SIGMA = 0.01*arma::eye(2,2);
    dklcost<SingleInt> cost (q,R,65,SIGMA,0,1,image,xbound,ybound,T,4.0,&syst1);
    sac<SingleInt,dklcost<SingleInt>> sacsys (&syst1,&cost,0.,T,umax,unom);
    migmda<SingleInt,dklcost<SingleInt>> demon(&sacsys, false);
    arma::vec xwrap;
    uniform_real_distribution<> user(-10,10);
    default_random_engine generator;
    arma::vec input = {user(generator),user(generator)};
    
    myfile<<"time,x,y,ux,uy,dklcost\n";
